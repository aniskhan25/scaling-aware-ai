...
#SBATCH --job-name=sai-ddp-config
...
: "${CONFIG:?Set CONFIG to a DDP training config path.}"

singularity exec "$CONTAINER" bash -lc "
set -euo pipefail
cd '${SLURM_SUBMIT_DIR:-$PWD}'
python scripts/summarize_environment.py --config '$CONFIG'
python -m torch.distributed.run --standalone --nnodes=1 --nproc_per_node=8 scripts/inspect_placement.py --config '$CONFIG'
